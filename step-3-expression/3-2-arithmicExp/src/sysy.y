...
%token INT RETURN
%token <str_val> IDENT PLUS MINUS NOT MUL DIV MOD
%token <int_val> INT_CONST

// 非终结符的类型定义
%type <ast_val> Block Stmt Number 
// 定义运算相关
%type <ast_val> FuncDef FuncType Exp PrimaryExp UnaryExp MulExp AddExp
...
Exp 
  : AddExp {
    auto ue = dynamic_cast<AddExp*>($1);
    auto ast = new Exp();
    ast->add_exp = unique_ptr<AddExp>(ue);
    ast->lineno = @1.first_line; 
    $$ = ast; 
